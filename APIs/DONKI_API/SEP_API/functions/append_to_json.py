...
def append_to_json(data):
    ...
    existing_dates = [entry.get('sepID') for entry in existing_data]
    if data['sepID'] not in existing_dates:
        existing_data.append(data)
        with open(json_file_path, 'w', encoding='utf-8') as file:
            json.dump(existing_data, file, indent=2)
        logging.info("Data appended successfully.")
    else:
        logging.info(f"Data with date {data['sepID']} already exists. Skipping append.")